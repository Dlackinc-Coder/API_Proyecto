...
class PedidosController {
    static async crearPedido(req, res) {
        ...
        try {
            ...
            nuevoPedido = await Pedidos.CrearPedido(client, folio, parseInt(id_cliente, 10), parseFloat(total));
            ...
            if (productos && productos.length > 0) {
                for (const prod of productos) {
                    ...
                    await Inventario.RegistrarMovimiento(parseInt(prod.id_producto, 10), -parseInt(prod.cantidad, 10), MovementType.VENTA_CLIENTE, `Pedido ${folio}`, null, client);
                }
            }
            if (metodo_pago) {
                await client.query(`INSERT INTO transacciones_pago (id_pedido, proveedor, proveedor_order_id, monto)
           VALUES ($1, $2, $3, $4)`, [id_pedido, metodo_pago, folio, total]);
            }
    ...
    static async obtenerPedidoPorId(req, res) {
        ...
        try {
            const pedido = await Pedidos.ObtenerPedidoPorId(parseInt(id, 10));
            if (!pedido) {
    ...
    static async ActualizarEstado(req, res) {
        ...
        try {
            const pedidoActualizado = await Pedidos.ActualizarEstado(parseInt(id, 10), estado);
    ...
    static async eliminarPedido(req, res) {
        const { id } = req.params;
        try {
            await Pedidos.CancelarPedido(parseInt(id, 10));
            res.status(204).send();
        }
        catch (error) {
            console.error(error);
