    ...
    static async login(req, res) {
        ...
        try {
            ...
            if (usuario.bloqueado_hasta && new Date(usuario.bloqueado_hasta) > new Date()) {
                const minutosRestantes = Math.ceil((new Date(usuario.bloqueado_hasta).getTime() - Date.now()) / 60000);
            ...
            const contrasenaEsValida = await bcrypt.compare(password, usuario.password_hash);
           ...
           SET intentos_login_fallidos = intentos_login_fallidos + 1,
               ...
               END
           WHERE id_usuario = $1`, [usuario.id_usuario]);
