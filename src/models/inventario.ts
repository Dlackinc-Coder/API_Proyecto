...
class Inventario {
  ...
  static async ObtenerKardexPorProducto(id_producto: number): Promise<InventarioKardex[]> {
    const result = await pool.query(
      "SELECT * FROM inventario_kardex WHERE id_producto = $1 ORDER BY fecha DESC",
      [id_producto]
    );
    return result.rows;
  }
